...
namespace yiiComponent\restApi\generators\model;
...
class Generator extends \yii\gii\Generator
{
    ...
    public function generateRules($table)
    {
        ...
        foreach ($table->columns as $column) {
            ...
            $columnDefaultValue = (string)$column->defaultValue;
            switch ($columnDefaultValue) {
                case 'CURRENT_TIMESTAMP':
                    break;
                case '0':
                    echo '000';
                case '0.00':
                    $defaultValue['number'][] = $column->name;
                    break;
                case '':
                    if ($column->allowNull) {
                        $defaultValue['null'][] = $column->name;
                    } else {
                        $defaultValue['stringEmpty'][] = $column->name;
                    }
                    break;
                default:
                    $defaultValue["$column->defaultValue"][] = $column->name;
            }
        ...
        foreach ($types as $type => $columns) {
            ...
            switch ($type) {
                case 'integer':
                    $rules[] = "[['" . implode("', '", $columns) . "'], '$type', 'min' => 0]";
                    break;
                case 'date':
                    $rules[] = "[['" . implode("', '", $columns) . "'], '$type', 'format' => 'yyyy-MM-dd']";
                    break;
                case 'time':
                    $rules[] = "[['" . implode("', '", $columns) . "'], '$type', 'format' => 'HH:mm:ss']";
                    break;
                case 'datetime':
                case 'timestamp':
                    $rules[] = "[['" . implode("', '", $columns) . "'], '$type', 'format' => 'yyyy-MM-dd HH:mm:ss']";
                    break;
                default:
        ...
        foreach ($defaultValue as $default => $cols) {

            switch ($default) {
                case 'null':
                    $rules[] = "[['" . implode("', '", $cols) . "'], 'default', 'value' => null]";
                    break;
                case 'stringEmpty':
                    $rules[] = "[['" . implode("', '", $cols) . "'], 'default', 'value' => '']";
                    break;
                case 'number':
                    $rules[] = "[['" . implode("', '", $cols) . "'], 'default', 'value' => 0]";
                    break;
                default:
                    $rules[] = "[['" . implode("', '", $cols) . "'], 'default', 'value' => $default]";
            }
